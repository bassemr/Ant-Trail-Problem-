...
class AntgameAI:
    ...
    def move_ant(self, direction) -> int:
        ...
        if (0 <= new_position[0] < self.N and 0 <= new_position[1] < self.N )  :
            ...
            self.score += self.visited[new_position]
            reward = self.visited[new_position]
            if reward == 1:
    ...
    def game_over(self) -> bool :
        ...
        """
        Checks if the game is over based on the game conditions.
    ...
    def undiscover_new_cells(self,x,y,temp_visited, cells)  -> np.ndarray:
        """
        Marks specified cells as undiscovered in the temp_visited array.

        Args:
        x (int): The x-coordinate of the ant.
        y (int): The y-coordinate of the ant.
        temp_visited (np.ndarray): The array tracking visited cells.
        cells (list): List of cells to mark as undiscovered.

        Returns:
        np.ndarray: Updated temp_visited array.
        """
        N = self.N
        m = self.m
        for nx,ny in cells:
    ...
    def discover_new_cells(self,x,y,temp_visited) -> tuple :
        """
        Discovers new cells around the ant's position and updates the visited array.
        ...
        Returns:
        tuple: Updated temp_visited array and a list of newly discovered cells.
        """
        N = self.N                
        m = self.m
        cell_discovered = []
    ...
    def update_visited(self):
        ...
        """
        Updates the visited array based on the ant's current position.
        """
        x, y = self.ant_position
    ...
    def draw_grid(self):
        ...
        for i in range(self.N):
            for j in range(self.N):
                ...
                if self.visited[i,j] != -np.inf:
                    self.screen.blit(street_image, rect)
                    if self.visited[i, j] == 1 and (i != x or j != y):
    ...
    """
        ...
        def backtrack(x, y, moves_left, current_score, current_path):
            nonlocal max_score, best_path, min_moves_left, grid
            ...
            for direction, dx, dy in directions:
                ...
                if 0 <= nx < N and 0 <= ny < N:
                    if grid[nx][ny] >= 0:
                        grid, cells = self.discover_new_cells(nx, ny, grid.copy())
                        potential_discovery = np.count_nonzero(grid == -np.inf)
                        next_moves.append((direction, nx, ny, potential_discovery))
                        grid = self.undiscover_new_cells(nx, ny, grid.copy(), cells)  # Revert to original grid state


            # Sort moves by grid value, preferring higher values
            next_moves.sort(key=lambda move: (-grid[move[1]][move[2]], move[3]) , reverse=False)
            for direction, nx, ny, _ in next_moves:
        ...
        #TODO check if there is undiscover cell
        max_discovered_cells = np.count_nonzero(self.visited == -np.inf)
        #max_score = self.score
        found = False

        def explore_directions(x, y, moves_left, current_path, current_discovery):
            nonlocal best_path, max_discovered_cells, grid, min_moves_left, found

            if current_discovery < max_discovered_cells or \
                current_discovery == max_discovered_cells and moves_left > min_moves_left:
                best_path = current_path.copy()
                max_discovered_cells = current_discovery
                min_moves_left = moves_left
                if current_discovery == 0:
                    found = True

            if moves_left == 0 or current_discovery ==  0 or \
                found and moves_left == min_moves_left and current_discovery> max_discovered_cells :#or found
                return 
        
                
                # If this move has less potential discovery, record it as the best path so far
            ...
            for direction, dx, dy in directions:
                nx, ny = x + dx, y + dy
                if 0 <= nx < N and 0 <= ny < N:
                    if grid[nx][ny] >= 0:  # Only consider cells that haven't been discovered
                        # Simulate the move
                        reward = grid[nx][ny]
                        grid[nx][ny] -= 1
                        grid, cells = self.discover_new_cells(nx, ny, grid.copy())
                        ...
                        potential_discovery = np.count_nonzero(grid == -np.inf)
                        # Collect moves with their potential discovery value
                        next_moves.append((direction, nx, ny, potential_discovery, reward, moves_left))
                        grid = self.undiscover_new_cells(nx, ny, grid.copy(), cells)
                        # Revert the simulated move
                        grid[nx][ny] += 1
                        


            # Sort by discovery potential, prioritize moves that discover more cells
            next_moves.sort(key=lambda move: (-move[4], move[3], -move[5]), reverse=False)
            # Attempt to make the best move
            next_moves = next_moves[0:1]
            for direction, nx, ny, potential_discovery, reward, _ in next_moves:
                if current_path and opposite_direction(current_path[-1], direction):
                    continue  # Skip moves that would backtrack
                ...
                # Recur with the updated position and remaining moves
                explore_directions(nx, ny, moves_left - 1, current_path, potential_discovery)

                # Undo the move after recursion
                grid = self.undiscover_new_cells(nx, ny, grid.copy(), cells)  # Revert to original grid state
                grid[nx][ny] += 1
                current_path.pop()
        ...
        def opposite_direction(prev_direction, current_direction):
            ...
            return opposites.get(prev_direction) == current_direction
...
def start_game() -> int:
    ...
    instructions = [
        ...
        "To begin, enter the grid size (N x N) and view range (m):"
    ]
    ...
    input_box_N = pygame.Rect(0, H - 120, 200, 40)
    input_box_m = pygame.Rect(0, H - 60, 200, 40)
    color_inactive = BLUE
    color_active = RED
    ...
    text_N = ''
    text_m = ''
    placeholder_N = 'Enter grid size'
    placeholder_m = 'Enter view range (m)'
    done = False
    ...
    while not done:
        ...
        # Render the current text or placeholder text for N
        if text_N:
            txt_surface_N = font.render(text_N, True, BLACK)
        else:
            txt_surface_N = font.render(placeholder_N, True, BLACK)

        # Render the current text or placeholder text for m
        if text_m:
            txt_surface_m = font.render(text_m, True, BLACK)
        else:
            txt_surface_m = font.render(placeholder_m, True, BLACK)
...
def print_instruction():
    ...
    print(f"The ant has a view of a square centered on its position with a size of (2 * m + 1).")
    print("This means you can discover new cells within this range.")
    print("Let's start!\n")
def run(N, m):
